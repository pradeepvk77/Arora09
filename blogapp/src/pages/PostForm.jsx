...
import { login, pushInPost } from "../reduxstore/reducer/authSlice";
import databaseCollection from "../service/database";
import { Editor } from "@tinymce/tinymce-react";
import bucketCollection from "../service/bucket";
import sendsvg from '../../assest/svg/sendsvg.svg'
...
function PostForm({ post }) {
    ...
    const { register, watch, control, handleSubmit, setValue, getValues } = useForm({
        defaultValues: {
            title: post?.title || "",
            slug: post?.slug || "",
    ...
    const submitPost = async (data) => {// at first editor give undefine if valued not typed yet
        setError(null);
        setSubmiting(true);
        const dataTitle = data.title.charAt(0).toUpperCase() + data.title.slice(1)
        data = { ...data, title: dataTitle, author: userData.name, authorId: userData.$id, slug: data.slug };
        if (!image) {
            setSubmiting(false);
        ...
        if (post) {
            ...
            await databaseCollection.upadatePostData({ ...data, imageId: imageId }, id);
        } else {
    ...
    return (
        <div className="bg-gray-300 bg-opacity-30 w-3/4 mx-auto rounded-lg p-2 mt-2">
            <form onSubmit={handleSubmit(submitPost)} className="flex flex-col justify-center item-center pb-4 m-2">
                <div className="flex flex-col flex-wrap">
                    ...
                    >Bind your thought in word</div>
                    <div className="flex flex-wrap flex-col">
                        <label className="text-2xl font-semibold py-2">Title</label>
                        <input type="text" placeholder="title of your blog" value={titleSet}{...register("title")} onChange={(e) => { setTitleSet(e.target.value) }}
                            className="h-10 text-xl pl-2 rounded-lg shadow-md" />
                        <label className="text-xl font-semibold py-2">Slug(id)</label>
                        <input type="text" placeholder="blog-id" disabled='true' {...register("slug")}
                            className=" h-10 text-xl pl-2 rounded-lg shadow-md" />
                    ...
                    <div className="flex flex-col  justify-center items-center p-2 py-4">
                        <span className="flex justify-center text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-black via-blue-900 to-purple-900 p-2">A image tell more than word.</span>
                        {image && <img src={URL.createObjectURL(image)} alt="Image" className="w-2/4 rounded-lg shadow-lg m-2" />}
                        <input type="file" onChange={(e) => { setImage(e.target.files[0]) }} />
                    </div>
                </div>
                <div className="loading flex flex-row justify-center items-center">
                    <button className="bg-blue-600 w-22 self-center flex rounded-lg px-2 py-1 pl-4 mr-4 text-xl font-semibold ">Post
                    <img src={sendsvg} alt="send" className="h-6 self-center mx-2" />
                    </button>
                    {error && <p className="m-2 text-red-600">{error}</p>}
                    {submiting && <span className="spinner"></span>}
                </div>
            </form>
        </div>
    )
